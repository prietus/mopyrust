...
const USER_AGENT: &str = "mopyrust/0.1.0 ( https://github.com/carlos/mopyrust )";
...
impl Wikipedia {
    ...
    /// Search for an artist by name (handles compound names + EN→ES fallback).
    ...
    /// Topic search: try exact slug, then fuzzy via search API.
...
fn urlencode_path(s: &str) -> String {
    // Path component: keep the underscores and most letters, encode spaces and unsafe chars.
    let mut out = String::with_capacity(s.len());
    for ch in s.chars() {
